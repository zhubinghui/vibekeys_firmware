...
use embedded_text::{plugin::Plugin, Token};

use crate::ansi_plugin::utils::try_parse_sgr;

mod utils;

/// Ansi sequence parser plugin.
#[derive(Clone)]
pub struct MyAnsiPlugin<'a, C: PixelColor> {
    carry: Option<Token<'a, C>>,
}

impl<C: PixelColor> MyAnsiPlugin<'_, C> {
...
impl<'a, C: PixelColor + From<Rgb888>> Plugin<'a, C> for MyAnsiPlugin<'a, C> {
    fn next_token(
        &mut self,
                ...
                Some((0, _)) => match ansi_parser::parse_escape(text) {
                    Ok((string, output)) => {
                        let new_token = match output {
                            ...
                            AnsiSequence::SetGraphicsMode(sgr) => {
                                try_parse_sgr(&sgr)
                                    .map(|sgr| {
                                        self.carry = Some(Token::Word(string));
